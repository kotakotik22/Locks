...
public final class LocksClientForgeEvents {
	...
	@SubscribeEvent
	public static void onRenderWorld(RenderLevelLastEvent e) {
		Minecraft mc = Minecraft.getInstance();
	...
	public static void renderLocks(PoseStack mtx, MultiBufferSource.BufferSource buf, Frustum ch, float pt) {
		...
		for (Lockable lkb : mc.level.getCapability(LocksCapabilities.Instances.LOCKABLE_HANDLER).orElse(null).getLoaded().values()) {
			...
			// todo (kota): im not sure the seed (the last argument) should be 0 here
	...
	public static void renderHudTooltip(PoseStack mtx, List<? extends FormattedCharSequence> lines, Font font) {
		...
		// todo (kota): im not sure if just removing shadeModel is fine
...
//		RenderSystem.shadeModel(7424);
		...
		var buf1 = MultiBufferSource.immediate(Tesselator.getInstance().getBuilder());

		Matrix4f last = mtx.last().pose();
		for (int a = 0; a < lines.size(); ++a) {
			var line = lines.get(a);
			if (line != null)
